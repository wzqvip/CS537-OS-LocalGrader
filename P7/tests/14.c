...
const int expected_data_block_count = 21;
...
int main() {
  ...
  for (size_t i = 0; i < filesize; i += BLOCK_SIZE) {
    CHECK(write_file_check(fd1, buf1 + i, BLOCK_SIZE, "mnt/large1.txt", i));
    CHECK(write_file_check(fd2, buf2 + i, BLOCK_SIZE, "mnt/large2.txt", i));
  }
