...
// ====================================================================
// Test 26
// Summary: Validates parent and child can see each others changes for anonymous shared mappings with fork
//
...
int main() {
    ...
    int pid = fork();
    if (pid == 0) {
        // child process waits for 20 ticks, so that the parent can write to the mapping
        sleep(20);
        ...
        char *arr3 = (char *)map;
        for (int i = 0; i < len; i++) {
            if (arr3[i] != parent_newval) {
                printf(1, "Cause: Child sees '%c' at Map 1, but expected '%c'\n", arr3[i], parent_newval);
                failed();
            }
        }
        printf(1, "Child sees the parent-modified data in Map 1. \tOkay\n");
        ...
        printf(1, "Parent modified the data in Map 1. \tOkay\n");
        ...
        char *arr = (char *)map;
        for (int i = 0; i < len; i++) {
            if (arr[i] != child_newval) {
                printf(1, "Cause: Parent sees %d at Map 1, but expected %d\n", arr[i], child_newval);
                failed();
            }
        }
        printf(1, "Parent sees the child-modified data in Map 1. \tOkay\n");
