...
// ====================================================================
// Test 23
// Summary: Validates parent and child can NOT see eachothers change for anonymous private mappings with fork
//
// Checks whether the parent process sees the child's modification
// Checks whether the child process sees the parent's modification
...
int main() {
    ...
    uint map = wmap(addr, len, fixed_anon_private, fd);
    if (map != addr) {
    ...
    printf(1, "Wrote '%c' to all pages of Map 1. \tOkay\n", val);
    ...
    if (pid == 0) {
        ...
        // child process waits for 100 ticks, so that the parent can write to the mapping
        sleep(200);
        printf(1, "Child waited for 200 ticks. \tOkay\n");

        //
        // 3. the child process should see the old data at the time of forking in Map 1
        //
        char *arr3 = (char *)map;
        for (int i = 0; i < len; i++) {
            if (arr3[i] != val) {
        ...
        printf(1, "Child modified the data to '%c' in Map 1. \tOkay\n", child_newval);
        ...
        // modify the data in Map 1
        for (int i = 0; i < len; i++) {
            arr[i] = parent_newval;
        }
        printf(1, "Parent modified the data to '%c' in Map 1. \tOkay\n", parent_newval);
