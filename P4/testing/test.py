...
class Test(object):
    ...
    def runexe(
        ...
        start = time()
        child = self.startexe(args, libs, path, stdin=infd, stdout=outfd, stderr=errfd)
        ...
        if status is not None and status != child.returncode:
            raise Failure(
                str(name)
                + " returned incorrect status code. "
                + "Expected "
                + str(status)
                + " got "
        ...
        if stdout is not None and stdout != outdata:
            raise Failure(
                str(name) + " gave incorrect standard output.", diff(stdout, outdata)
            )
        if stderr is not None and stderr != errdata:
            raise Failure(
                str(name) + " gave incorrect standard error.", diff(stderr, errdata)
            )

        return child, outdata

    def startexe(
        self, args, libs=None, path=None, stdin=None, stdout=None, stderr=None, cwd=None
    ):
        name = args[0]
        if stdout is None:
            stdout = self.logfd
        ...
        if self.use_gdb:
            child = subprocess.Popen(
                ["xterm", "-title", '"' + " ".join(args) + '"', "-e", "gdb", "--args"]
                + args,
                cwd=cwd,
                stdin=stdin,
                stdout=stdout,
    ...
    def run_util(self, args):
        self.log(" ".join(args))
        child = subprocess.Popen(
            args, cwd=self.project_path, stdout=self.logfd, stderr=self.logfd
        )
        self.children.append(child)
        status = child.wait()
        return status
    ...
    def run_filter(self, args):
        self.log(" ".join(args))
        child = subprocess.Popen(
            args, cwd=self.project_path, stdout=subprocess.PIPE, stderr=None
        )
...
def run_test(test, queue):
    ...
    try:
        ...
        # Nullify test logfd for serialization
        test.logfd = None
        queue.put(test)
        sys.exit()
