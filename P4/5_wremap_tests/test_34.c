...
int main() {
    ...
    get_n_validate_wmap_info(&winfo5, 0);       // no map exists
    map_exists(&winfo5, mapv3, length3, FALSE); // Map 1 does not exist
    struct pgdirinfo pinfo;
    get_n_validate_pgdirinfo(&pinfo);
    va_exists(&pinfo, mapv3 + offset, FALSE);
